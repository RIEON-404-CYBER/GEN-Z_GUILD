...
{Fore.YELLOW}║     RIEON'S FREE FIRE BOT         ║{Fore.RESET}
{Fore.YELLOW}║    Powered by GEN-Z x64 (Guild)   ║{Fore.RESET}
{Fore.CYAN}╚════════════════════════════════════╝{Fore.RESET}
    """
    for char in banner:
...
def send_likes(uid):
    try:
        url = f"http://127.0.0.1:5000/like?uid={uid}&server_name={server2}&key={key2}"
        ...
        if likes_api_response.status_code == 200:
            ...
            likes_before = int(api_json_response.get('likes_before', 0))
            likes_after = int(api_json_response.get('likes_after', 0))
            likes_added = int(api_json_response.get('likes_added', 0))
...
[FFFFFF]Likes Before : [FFB300]{likes_before}  
[FFFFFF]Likes After  : [00FF00]{likes_after}  
[FFFFFF]Likes Added  : [00FF00]+{likes_added}  
[C][B][11EAFD]‎━━━━━━━━━━━━
[C][B][FFB300]Credits: [FFFFFF]RIEON [00FF00]!!
"""
                ...
                if "8" in parsed_data["5"]["data"] and "data" in parsed_data["5"]["data"]["8"]:
                    ...
                    if uexmojiii == "DefaultMessageWithKey":
                        ...
                        clients.send(
                            self.GenResponsMsg(
                                f"""[C][B]
Welcome to [00ff00]RIEON'S BOT[ffffff]!
...
####################################
            ...
            if "1200" in data.hex()[0:4] and b"/info" in data:
                try:
                    ...
                    # API Call
                    url = f"https://like2.vercel.app/player-info?uid={uid}&server={server2}&key={key2}"
                    print(url)
                    response = requests.get(url)
                    print(response)
...
def run_client(id, password):
    ...
    loading("InitializingRIEON's Free Fire Bot", duration=3)
